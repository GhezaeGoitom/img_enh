...
class TrainModel:
    ...
    def srcnnModel(self,layer1, layer2, layer3):
    ...
    def train(self,layer1,layer2,layer3):
        ...
        data, label = self.read_data(orginalPath+"/data/processed/training/train.h5")
        val_data, val_label = self.read_data(orginalPath+"/data/processed/training/test.h5")
...
if __name__ == "__main__":
    ...
    for row in params:
        # Start mlflow run
        mlflow.start_run()
        TrainModel().train(row[0],row[1],row[2])
        # End mlflow run
        mlflow.end_run()
